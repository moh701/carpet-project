...
<body>
<div class="container">
  <form method = "POST" id = "producer-forgetpassword-form" action="<c:url value="/producers/forgetPassword/"/>" class="row g-3 position-absolute top-50 start-50 translate-middle">
    <div class="form-floating mb-3 text-center">
      <h3>Producer Reset Password</h3>
    </div>
    ...
    <c:if test="${not empty success}">
      <div class="col-12 text-center">
        ...
        <a href = "<c:url value="/producers/login/"/>" class="ms-2">Click here for login</a>
      </div>
    </c:if>
    <c:if test="${not empty error}">
      <div class="col-12 text-center">
        <p id="error" class="text-danger">${error}</p>
        <a href = "<c:url value="/producers/register/"/>" class="ms-2">Click here for register</a>
      </div>
    </c:if>
  </form>
</div>
